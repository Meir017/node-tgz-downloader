 ...
 * @returns { Promise<Set<string>> }
 */
    async getDeepDependencies(dependenciesObject,options) {
      const packageJson = await this._retrievePackageVersion(dependenciesObject);
      if (!packageJson) {
        logger(['ERROR'.red], 'failed to retrieve version of package', dependenciesObject.name, dependenciesObject.version);
        return new Set();
      }
      if (this.tarballs.has(packageJson.dist.tarball)) return this.tarballs;
      ...
      await this._getDeepDependenciesFrom(packageJson.dependencies, options);
  
      if (options.devDependencies) {
        await this._getDeepDependenciesFrom(packageJson.devDependencies, options);
      }
  
      if (options.peerDependencies) {
        await this._getDeepDependenciesFrom(packageJson.peerDependencies, options);
      }
   ...
   * @param { PackageJsonDependenciesOptions } options
   */
    async getDeepPackageJsonDependencies(packageJson,options) {
  ...
  async _getDeepDependenciesFrom(dependenciesObject, options) {
    const dependencies = Object.keys(dependenciesObject || {});
    await Promise.all(dependencies.map(dependency => {
      let name  =  dependency;                                                
      let version  =  dependenciesObject[dependency]; 
      ...
      return this.getDeepDependencies({
      name: name,
      version: version
    },options)}));
  }
